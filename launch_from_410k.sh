#!/bin/bash
# Lancer avec checkpoints 410k pour GPU 0,3,4,5 et nouveaux pour GPU 1,2

echo "═══════════════════════════════════════════════════════════════════"
echo "  🚀 REPRISE DEPUIS STEP 410,000"
echo "═══════════════════════════════════════════════════════════════════"
echo ""
echo "  GPU 0, 3, 4, 5: Reprise depuis step 410,000"
echo "  GPU 1, 2:       Nouveau démarrage"
echo ""
...
nohup python src/train.py \
  ...
  > logs/gpu0.log 2>&1 &
echo "GPU 0: PID $! (resume from 410k)"
...
nohup python src/train.py \
  ...
  --checkpoint_every 10000 --log_every 100 \
  > logs/gpu2.log 2>&1 &
echo "GPU 2: PID $! (nouveau - step 0)"

# GPU 3 - Reprise depuis 410k
...
nohup python src/train.py \
  ...
  > logs/gpu3.log 2>&1 &
echo "GPU 3: PID $! (resume from 410k)"

# GPU 4 - Reprise depuis 410k
...
nohup python src/train.py \
  ...
  > logs/gpu4.log 2>&1 &
echo "GPU 4: PID $! (resume from 410k)"

# GPU 5 - Reprise depuis 410k
...
nohup python src/train.py \
  ...
  > logs/gpu5.log 2>&1 &
echo "GPU 5: PID $! (resume from 410k)"

sleep 5

echo ""
echo "═══════════════════════════════════════════════════════════════════"
echo "✅ 6 MODÈLES LANCÉS!"
echo "═══════════════════════════════════════════════════════════════════"
echo ""
echo "📊 Configuration:"
echo "   GPU 0: Step 410,000 → 757,500 (347,500 restants)"
echo "   GPU 1: Step 0 → 757,500 (nouveau)"
echo "   GPU 2: Step 0 → 757,500 (nouveau)"  
echo "   GPU 3: Step 410,000 → 757,500 (347,500 restants)"
echo "   GPU 4: Step 410,000 → 757,500 (347,500 restants)"
echo "   GPU 5: Step 410,000 → 757,500 (347,500 restants)"
echo ""
echo "💾 Checkpoints: tous les 10,000 steps"
echo "🧹 Auto-cleanup: 3 derniers uniquement"
echo ""
